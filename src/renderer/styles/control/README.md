...
| 9 | `shared-components.css` | Cross-surface primitives: `.db-control`, `.badge`, `.status-footer`, `.sr-only`, `.icon-button`, `.control-icon` |

Keep **stream** before **shared** so stream-scoped rules that reference `.icon-button` / `.db-control` stay ahead of the generic component rules (same specificity discipline as the legacy monolith).
